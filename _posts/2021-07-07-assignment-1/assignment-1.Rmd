...
packages = c('tidytext','widyr','wordcloud',
             ...
             'visNetwork', 'topicmodels', 'crosstalk', 'utf8', 'knitr')
...
3. Next, an observation from the network analysis showed that 2 staff have the most number of correspondence with the employees. They are namely, Linnea Bergan and Ruscella Mies Haber from the administration team. Upon closer inspection in the email headers, we can conclude that they typically send group emails to the employees with regard to administrative and Information Technology related topics. As such, the information might not be as useful in determining relationships and will be excluded in the analysis below.  
...
It seemed that Sten has personal relationships with Ada Campo, Orhan Strum and William Vasco as there were emails going back and forth with regards to an impromptu golf game and seeking for recommendation for a good vacation spot. Ingrid Barranco only spoke to Sten with regard to work events only.

Another correspondence is with Henk Mies, a Truck Driver from the Facilities department. Unfortunately, we only have the subject title of the email and is only labelled files. We will assume that this is a work incident that requires CEO's attention. 
